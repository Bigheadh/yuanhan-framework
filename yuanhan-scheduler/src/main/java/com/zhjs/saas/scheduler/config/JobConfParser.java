...
import com.yuanhan.yuanhan.scheduler.service.JobService;
...
public class JobConfParser implements ApplicationContextAware
{
	...
	public void setApplicationContext(ApplicationContext ctx) throws BeansException
	{
		...
		Map<String, Object> beanMap = ctx.getBeansWithAnnotation(JobTask.class);
		for (Object confBean : beanMap.values())
		{
			...
			JobTask conf = clz.getAnnotation(JobTask.class);
			...
			// 任务执行日志数据源，以名称获取
			if (StringUtil.isNotBlank(eventTraceRdbDataSource))
			{
	...
	private List<BeanDefinition> getTargetElasticJobListeners(JobTask conf)
	{
		List<BeanDefinition> result = new ManagedList<BeanDefinition>(2);
		String listeners = getEnvironmentStringValue(conf.name(), JobAttributeTag.LISTENER, conf.listener());
		if (StringUtil.isNotBlank(listeners))
		{
		...
		if (StringUtil.isNotBlank(distributedListeners))
		{
